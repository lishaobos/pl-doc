...
export default {
  ...
  methods: {
    ...
    async focus (e) {
      this.$emit('focus', e)
      await this.$nextTick()
      let spans = document.querySelectorAll('.el-picker-panel__footer span') || []
    ...
    sort ({ start, end }) {
      ...
      let s = moment(start).valueOf()
      let e = moment(end).valueOf()
      let format = this.momentType[this.type]
      if (e < s) {
        let s = moment(end).format(format)
        let e = moment(start).format(format)
        let date
        /**
         * 根据当前格式化类型进行调换
         * date类型需要手动拼接时间
